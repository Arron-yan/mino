...
- **(2026-09-10) 会给用户拿去行动的技术结论，先验证再说** — 本会话两次把未验证的判断当事实告知：① 建议 Arron 开 Zotero 本地 API 时说"开了我就能建分类/删条目"，他据此真的去开了，实测那 API 是**只读**的（DELETE 501）；② 从 profile 路径推断"这浏览器是你本人的日常 Chrome"，被纠正（是 MyAgents 内置浏览器）。**Why:** 用户会按你说的去做，落空的成本高于实测的成本。**How to apply:** 涉及"某接口能不能做 X""某环境属于谁"这类判断，能实测就先实测；确实没条件验的，明说这是推断。
- **(2026-09-10) Arron 会追问"为什么这么绕"** — 本会话两次质疑实现比必要的更复杂：建分类那步（"对我来说操作很容易…为什么这么复杂"）、下 PDF 要先落盘（"为什么要下载那几个文件，不能直接通过 Zotero 的保存功能直接保存下来吗"）。两次追问都指向真问题，改完确实更简单。**How to apply:** 优先走原生/直路，别为绕开限制自造中转；被追问时去查清机制再答（两次都是靠扒源码/实测才答准），不要辩解或用"这是限制"含糊过去。
- **(2026-09-10) 写产物前先查自己已有的记忆与技能** — 要写"登录流程"skill 时，我跑去浏览器重新探测威科登录入口，被 Arron 一句"**这些登陆的过程不是在你的 memory 里吗**"点破：流程本来就完整记在 `memory/topics/legal-research-infra.md`。**Why:** 记忆和 skill 是我的资产，重新探测既慢又容易把已有的正确记录写歪。**How to apply:** 要新建脚本/skill/流程文档前，**先读相关 topic 与已有 skill**，在既有记录上做增量修正（本次确实发现旧记录有偏差，但那是**复查后修正**，不是重新发明）。与上一条"为什么这么绕"同源。
...
- **(2026-09-08, updated 2026-09-10) 法律检索本地底座（三库登录态 / playwright-direct / 无视觉）** — **要登录或恢复登录态 → 加载 Skill `.claude/skills/legal-db-login/`**（2026-09-10 建；含 CHECK / 重登 / 按域清 cookie / 导出 / 恢复 的可粘贴片段）；**背景机制**读 `memory/topics/legal-research-infra.md`。**操作细节都在那两层，此处只留影响行为的结论**：
  - **法宝会话最脆**（Keycloak session cookie，浏览器进程一回收就掉线；威科/知网是持久 cookie 更耐）→ 只有法宝掉线是常态，单独重登它；**登录成功后立刻导出**。
  - **法宝把 CARSI 的天财身份映射到了手机号个人号上**（Arron 确认）→ **自己换号换不掉，别再试**；要独立学校号须找法宝客服。
  - **"cookie 没了" ≠ "要重新输密码"**：天财 SSO 服务端会话比浏览器 cookie 活得久，直接试走一遍（2026-09-10 实证）。天财账号密码**永远由 Arron 亲输，我不碰**。
  - **威科会弹点击式验证码** → 无视觉解不了，**停手告诉 Arron 让他点**（他已明确认可此边界）。
  - 检索走 `playwright-direct` MCP（内置 playwright 因 MyAgents 自带 npm 损坏不可用），配置指向 npx 缓存路径，**清 npm 缓存即失效**；**改 MCP 配置后必须完全重启应用 + 新建会话才生效**。
  - **当前 Runtime 无视觉** —— Read 截图一律返回 `[Unsupported Image]`；只能读 DOM 文本 / cookie 元数据，或走 cuse `ui snapshot`、`workspace/ocr-*.mjs` 离线 OCR。
...
- [[legal-research-infra]] — 法律库检索的登录态与浏览器底座（2026-09-08，更新 2026-09-10）。三库天财 CARSI 登录、登录态文件与恢复、检索入口/选择器、playwright-direct、cuse+OCR 读屏。**做任何判例/论文/法规检索前先读它**；**要动手登录则加载 Skill `.claude/skills/legal-db-login/`**。详见 `memory/topics/legal-research-infra.md`。
