...
- **(2026-09-08, updated 2026-09-10) 法律检索本地底座（三库登录态 / playwright-direct / cuse+OCR）** — **要登录或恢复登录态 → 加载 Skill `.claude/skills/legal-db-login/`**（2026-09-10 建，含可粘贴的 `browser_run_code` 片段：CHECK / 重登 / 按域清 cookie / 导出 / 恢复）；**背景机制**读 `memory/topics/legal-research-infra.md`。两条 09-10 新事实：①**法宝会话最脆**（依赖 Keycloak session cookie，浏览器进程一回收就掉线；威科/知网是持久 cookie 所以更耐）→ CHECK 只有法宝掉线是常态，单独重登它；**登录成功后立刻导出**。②**法宝把 CARSI 的天财身份映射到了手机号个人号上**（Arron 确认）→ **自己换号换不掉，别再试**，要独立学校号得找法宝客服。三条硬事实：①三库（威科/法宝/知网）走天财 CARSI，**先登任一库留下 SSO 会话，其余库免输号一跳即过**；登录态统一在 `workspace/legal-dbs-login.json`（已 gitignore，**内含会话凭证勿外传**），**时效约 7 天**，过期引导 Arron 重新登录后覆盖导出。②检索走 `playwright-direct` MCP（内置 playwright 因 MyAgents 自带 npm 损坏不可用），其配置指向 npx 缓存路径，**清 npm 缓存即失效**；**改 MCP 配置后必须完全重启应用 + 新建会话才生效**。③**当前 Runtime 无视觉**——Read 截图一律返回 `[Unsupported Image]`，只能靠 cuse `ui snapshot`（UIA 树）或 `workspace/ocr-image.mjs` / `ocr-words.mjs` 离线 OCR 读屏。
